...
The shared TradingView universe covers **US-listed common stocks plus US-listed depositary receipts (DRs)**. Downstream exchange gates stay unchanged, so OTC/PINK names remain excluded and the scanner still does **not** admit ETFs, funds, warrants, rights, or structured products.

### Freshness & refresh
Momentum names qualify and fade in seconds, so the universe is kept live: the table **auto re-scans every 30s** and shows a *"universe scanned Xs ago"* stamp. The server keeps only a short **30-second** result cache (keyed by the exact threshold set) purely to shield the unofficial TradingView scanner from duplicate/concurrent loads. The **Refresh** button forces a genuinely live scan by **bypassing that cache** (`?fresh=1` on `/api/ross` · `/api/largecap`), so you can always pull the current movers on demand. Live prices poll independently on a 30s / 1m / 5m selector.
...
### Signal Alignment (validation mode)
Fully qualified Ross rows show an explainable **`N/4 aligned`** checklist rather than a grade or price prediction. The four signals are: timestamp-verified fresh catalyst plus strong momentum, live pre/after-hours continuation, RVol acceleration measured across at least two minutes, and repeated qualification without significant fading. Watch rows show **Not yet qualified**, unavailable evidence remains **`?`**, unknown float is labeled **Low confidence**, and regular-session pre-market data is explicitly stale and earns no point.

Qualified snapshots persist the scan price, session, signal states, and alignment count in `data/screener-history.json`. Later scans settle the snapshot with the price/high/low and return metrics around **+30 minutes** using one-minute Yahoo candles. This outcome data is for validating whether higher alignment counts actually separate continuation performance; the UI does not sort by alignment or present it as a forecast.
...
### News & sentiment
Latest headlines per pick published **since the previous market close** (after-hours + pre-market catalyst window). Source is **Yahoo Finance** search by default, or **Finnhub** company-news (`lib/ross/finnhub.ts`) for near-real-time, minute-level breaking headlines when `FINNHUB_API_KEY` is set — otherwise the Finnhub path is a transparent no-op. Headlines pass through a keyword **sentiment** scorer that keeps bullish and neutrally worded catalyst candidates while excluding clearly negative stories; timestamp-less results remain visible but cannot earn the fresh-catalyst alignment point. Google Finance deep-links per row for manual research.

### Code
`config/ross.ts` · `config/largecap.ts` · `config/screenerProfile.ts` (shared profile abstraction) · `lib/ross/{tradingview,yahooFallback,extendedHours,pillars,sentiment,news,finnhub,alignment,outcomes,history,index}.ts` · `app/api/ross/route.ts` + `app/api/largecap/route.ts` (legacy `/api/screener` is a shim) · `components/ross/{RossTable,PillarBreakdown,RossControls,LargecapControls,BookTabs}.tsx`.
